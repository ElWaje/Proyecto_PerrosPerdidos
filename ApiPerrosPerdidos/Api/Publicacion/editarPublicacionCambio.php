...
if (isset($_GET['publicacion'])) {
    $selectedPublicacionId = $_GET['publicacion'];
    ...
    try {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            $selectedUserId = $_POST['idUsuario'] ?? null;

            if ($selectedUserId && $selectedPublicacionId) {
                $sqlActualizarIdUsuario = "UPDATE Publicaciones SET idUsuario = :idUsuario, idAutor = :idAutor WHERE id = :idPublicacion";
                $stmtActualizarIdUsuario = $conn->prepare($sqlActualizarIdUsuario);
                $stmtActualizarIdUsuario->bindParam(':idUsuario', $selectedUserId);
                $stmtActualizarIdUsuario->bindParam(':idAutor', $selectedUserId);
                $stmtActualizarIdUsuario->bindParam(':idPublicacion', $selectedPublicacionId);
                $stmtActualizarIdUsuario->execute();

                header('Location: ../admin.html');
                exit;
            } else {
                handleErrors("Datos incompletos para actualizar la publicación.");
            }
        }
    ...
    handleErrors("ID de publicación no especificado.");
}
?>
...
<head>
    ...
    <title>Editar Publicación Cambio</title>
...
<body>
    <header class="header">
        <div class="nav-container">
            <ul class="nav-links">
                ...
                <li><a href="editarPublicacionFormulario.php">Volver</a></li>
    ...
    <div class="contenido">
        ...
        <div class="editar-publicacion-container">
            ...
            <form action="editarPublicacionCambio.php?publicacion=<?php echo htmlspecialchars($selectedPublicacionId); ?>" method="POST">
